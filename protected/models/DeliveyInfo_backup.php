...
class DeliveyInfo extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('fio, date, status_id, weight','china_id','adress', 'required'),
			array('status_id, weight', 'numerical', 'integerOnly'=>true),
			array('fio', 'length', 'max'=>255),
            array('china_id', 'length', 'max'=>125),
            array('adress', 'length', 'max'=>255),
            array('ems_id', 'length', 'max'=>125),
			...
			array('id, fio, date, status_id,china_id, ems_id, weight, code', 'safe', 'on'=>'search'),
		);
	}
